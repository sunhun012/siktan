...
- 누구나 글을 남길 수 있습니다
- 글쓴이는 **자기가 남긴 글과 거기 달린 답장만** 봅니다 (기기에 저장된 작성자 표식으로 찾습니다. 계정은 없습니다)
- 관리자만 전체를 보고, 답장하고, 지웁니다
...
**의견을 읽으려면** Netlify에서 환경변수 `ADMIN_KEY`를 정한 뒤(설정 후 재배포 필요),
앱 설정의 '관리자 키' 칸에 같은 값을 넣으세요. 그 기기에서만 글 목록과 지우기 버튼이 보입니다.
키가 없으면 목록 조회(`GET /api/feedback`)는 서버에서 403으로 막힙니다.
